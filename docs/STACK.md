...
| **Nherit** | [nherit](https://github.com/JAFairweather/nherit) | Family break-glass digital legacy vault: one self-maintained estate record, per-beneficiary scopes, heirs and executors holding revocable grants; the whole estate reconstitutes from a single key printed on paper. | *Succession and time*: grants designed to outlive their grantor — honest break-glass tiers ("cryptography cannot verify death"), clean cut-offs, recovery as a sheet of paper instead of a support ticket to a company that may not exist in thirty years. |
| **Ntrigue** | [ntrigue](https://github.com/JAFairweather/ntrigue) | A phones-only party game of secrets, dilemmas, and blackmail: every answer is an encrypted scope; pairwise prisoner's-dilemma exchanges decide who sees what; the finale is a blackmail economy. | *Adversarial multiplayer*: mutually distrusting peers trading scopes as stakes, public relays as the game room, no server and no accounts. Its design law — "you can revoke a secret, but you can't un-tell it" — is §2.4's honesty clause turned into a game rule. |
| **Notegate** | [notegate](https://github.com/JAFairweather/notegate) | Serverless secure tip intake for newsrooms: a tip line is a keypair; sources are ephemeral browser-minted keys whose only reply channel is a 12-word phrase; proof-of-work is the spam toll, checked before decryption. | *Anonymous, asymmetric intake*: no accounts on either side, relays see only ciphertext from throwaway keys with fuzzed timing, and the Grant Index IS the case docket — the whole newsroom reconstitutes from the intake nsec alone. |
