...
export async function grant(relay, publisherSecret, granteePubkey,
  ...
  if (terms) content.nvoy = { nvoy: 1, ...terms }   // nvoy terms extension — vanilla clients ignore it
  const rumor = {
    pubkey: publisherPub,
    kind: KIND_GRANT,
    created_at: now(),
...
export async function receiveGrants(relay, granteeSecret) {
  ...
  for (const wrap of wraps) {
    ...
    const { scope_key, scope_name, nvoy } = JSON.parse(rumor.content)
    grants.push({
      publisher, scopeId, scopeName: scope_name, relayHint, nvoy: nvoy ?? null,
